    ...
    return (
        <div className="flex flex-col h-full bg-sidebarBackground">
            <div className="p-6 border-b">
                <h2 className="font-semibold text-xl truncate">Чат с {chat.userName}</h2>
            </div>
            ...
            <div className="flex-1 p-6 overflow-y-auto flex flex-col space-y-4">
                {chat.messages.map(message => (
                    <div
                        key={message.id}
                        className={`${
                            message.isFromUser ? 'self-end bg-accentColor rounded-[12px_12px_0_12px]' : 'self-start bg-accentColorLightOpposite shadow-sm rounded-[12px_12px_12px_0]'
            ...
            <div className="p-6 border-t bg-sidebarBackground flex items-center gap-3">
                ...
                <button className="px-6 py-3 rounded-xl bg-accentColor text-black hover:opacity-80 transition-opacity duration-300 font-medium whitespace-nowrap">
                    Отправить
                </button>
            </div>
        </div>
